...
const contentStem = computed(() => `solutions/${locale.value}/8.mes`)

const { data: postValue } = await useAsyncData(
  () => `solutions-mes-${locale.value}`,
  () => queryCollection('content').where('stem', '=', contentStem.value).first()
)

const title = t('solutions.mes_title')
const description = t('solutions.mes_description')
...
const pageUrl = computed(() => `https://www.aie-tec.com.tw${localePath('/solutions/mes')}`)
